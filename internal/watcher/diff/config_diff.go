...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	} else if !reflect.DeepEqual(summarizeAPIKeys(oldCfg.APIKeys), summarizeAPIKeys(newCfg.APIKeys)) {
...
func summarizeAPIKeys(in config.APIKeyEntries) []string {
	out := make([]string, len(in))
	for i := range in {
		entry := in[i]
		allowed := append([]string(nil), entry.AllowedModels...)
		out[i] = fmt.Sprintf(
			"%s|%s|%t|%s|%s|%d",
			strings.TrimSpace(entry.APIKey),
			strings.TrimSpace(entry.CustomerName),
			entry.Enabled,
