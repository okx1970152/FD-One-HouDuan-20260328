...
func TestBuildConfigChangeDetails_SecretsAndCounts(t *testing.T) {
	oldCfg := &config.Config{
		SDKConfig: sdkconfig.SDKConfig{
			APIKeys: config.APIKeyEntries{{APIKey: "a", Enabled: true}},
		},
		AmpCode: config.AmpCode{
			UpstreamAPIKey: "",
		},
	...
	newCfg := &config.Config{
		SDKConfig: sdkconfig.SDKConfig{
			APIKeys: config.APIKeyEntries{{APIKey: "a", Enabled: true}, {APIKey: "b", Enabled: true}, {APIKey: "c", Enabled: true}},
		},
		AmpCode: config.AmpCode{
...
func TestBuildConfigChangeDetails_FlagsAndKeys(t *testing.T) {
	oldCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			...
			APIKeys:                    config.APIKeyEntries{{APIKey: "key-1", Enabled: true}},
	...
	newCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			...
			APIKeys:                    config.APIKeyEntries{{APIKey: " key-1 ", Enabled: true}, {APIKey: "key-2", Enabled: true}},
...
func TestBuildConfigChangeDetails_AllBranches(t *testing.T) {
	oldCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			RequestLog: false,
			ProxyURL:   "http://old-proxy",
			APIKeys:    config.APIKeyEntries{{APIKey: " keyA ", Enabled: true}},
		},
	...
	newCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			RequestLog: true,
			ProxyURL:   "http://new-proxy",
			APIKeys:    config.APIKeyEntries{{APIKey: "keyB", Enabled: true}},
		},
