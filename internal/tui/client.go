...
func (c *Client) GetAPIKeys() ([]string, error) {
	...
	for _, entry := range objects {
		key := strings.TrimSpace(fmt.Sprint(entry["api-key"]))
		if key == "" || key == "<nil>" {
			continue
		}
...
func (c *Client) AddAPIKey(key string) error {
	_ = key
	return fmt.Errorf("commercial api keys must be created from the web management UI")
}

// EditAPIKey replaces an API key at the given index.
func (c *Client) EditAPIKey(index int, newValue string) error {
	_, _ = index, newValue
	return fmt.Errorf("commercial api keys must be edited from the web management UI")
}
...
func (c *Client) DeleteAPIKey(index int) error {
	...
	_, code, err := c.doRequest("DELETE", fmt.Sprintf("/v0/management/api-keys?api-key=%s", url.QueryEscape(keys[index])), nil)
	if err != nil {
		return err
	}
	if code >= 400 {
