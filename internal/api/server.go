...
func (s *Server) setupRoutes() {
	...
	v1.Use(apiKeyModelPolicyMiddleware())
	{
	...
	v1beta.Use(apiKeyModelPolicyMiddleware())
	{
	...
	s.engine.POST("/v1internal:method", AuthMiddleware(s.accessManager), apiKeyModelPolicyMiddleware(), geminiCLIHandlers.CLIHandler)
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.POST("/api-keys", s.mgmt.CreateAPIKey)
		mgmt.PUT("/api-keys", s.mgmt.PutAPIKeys)
		mgmt.PATCH("/api-keys", s.mgmt.PatchAPIKeys)
		mgmt.DELETE("/api-keys", s.mgmt.DeleteAPIKeys)
		mgmt.POST("/api-keys/extend", s.mgmt.ExtendAPIKey)
		mgmt.GET("/api-keys/usage", s.mgmt.GetAPIKeyUsageStatistics)
	...
	return func(c *gin.Context) {
		...
		if err == nil {
			if result != nil {
				...
				if !policy.ExpiresAt.IsZero() && !policy.ExpiresAt.After(time.Now().UTC()) {
